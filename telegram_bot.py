...
async def start_telegram_bot(discord_bot_instance, webhook_url: str = None):
    """Start the Telegram bot with webhook (no polling conflicts!)"""
    ...
    print("[OK] Telegram bot starting with webhook...")
    await telegram_app.initialize()
    
    # Set up webhook instead of polling (avoids conflicts!)
    if webhook_url:
        full_webhook_url = f"{webhook_url}{WEBHOOK_PATH}"
        await telegram_app.bot.set_webhook(url=full_webhook_url, drop_pending_updates=True)
        print(f"[OK] Telegram webhook set: {full_webhook_url}")
    else:
        # Fallback: delete webhook and don't poll (bot will only send, not receive)
        await telegram_app.bot.delete_webhook(drop_pending_updates=True)
        print("[OK] Telegram bot ready (webhook mode, waiting for URL)")
